...
namespace Common
{
   ...
   Math::AngleUnit Math::getAngleUnit(void)
   {
		...
		const Vector2& b, const Vector2& c)
    {
		...
		zeroDot[0] = Math::RealEqual(dot[0], 0.0f, static_cast<float>(1e-3));
		...
		zeroDot[1] = Math::RealEqual(dot[1], 0.0f, static_cast<float>(1e-3));
		...
		zeroDot[2] = Math::RealEqual(dot[2], 0.0f, static_cast<float>(1e-3));
		// Compare signs (ignore colinear / coincident points)
